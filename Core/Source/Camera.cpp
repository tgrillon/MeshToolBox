...
std::pair<float, float> Camera::PanSpeed() const
{
	...
	return { xFactor * m_TranslationSpeed, yFactor * m_TranslationSpeed };
}
	...
	if(Input::IsKeyPressed(Key::LeftAlt))
	{
		...
		else if(Input::IsMouseButtonPressed(Mouse::ButtonRight))
		{
			if(std::abs(delta.x) > std::abs(delta.y * m_AspectRatio))
				...
				MouseZoom(delta.y * m_AspectRatio);
		}
	}
	UpdateView();
}
...
Core::BaseType::Vec3 Camera::GetLightDirection() const
{
	return m_FocalPoint - m_Position;
}

void Camera::RenderImGuiWindow()
{
	ImGui::Begin("Camera settings");
	ImGui::SliderFloat("FOV", &m_FOV, 35.f, 105.f, "%.0f");
	ImGui::SliderFloat("Translation speed", &m_TranslationSpeed, 1.f, 5.f, "%.0f");
	ImGui::SliderFloat("Rotation speed", &m_RotationSpeed, 1.f, 4.f, "%.0f");
