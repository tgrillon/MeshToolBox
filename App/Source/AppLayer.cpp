...
void AppLayer::OnRender()
{
	...
	glUniformMatrix4fv(glGetUniformLocation(m_Shader, "u_Mvp"), 1, GL_FALSE, glm::value_ptr(mvp));

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, m_Texture.Handle);
	glUniform1i(glGetUniformLocation(m_Shader, "u_Texture"), 0);
	glUniform3fv(glGetUniformLocation(m_Shader, "u_Light"), 1, &m_Camera.GetLightDirection()[0]);
